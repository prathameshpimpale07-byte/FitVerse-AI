...
exports.sendContactEmail = async (req, res) => {
  try {
    ...
    // 1. Save message to database with fast 2s timeout safeguard
    try {
      await Promise.race([
        Contact.create({
          name: fullName,
      ...
      console.warn(`[Contact] DB save notice:`, dbErr.message);
    }

    // 2. Return success response immediately (< 100ms) to user
    res.status(200).json({ 
      success: true, 
      message: 'Message sent successfully!'
    });

    // 3. Attempt to send email asynchronously in background
